...
def detect_circles(img, canny_param=20, hough_param=20):
    ...
    circle_frame = cv.circle(
        img, (inner_circle[0], inner_circle[1]), inner_circle[2], (0, 0, 0), cv.FILLED)
...
#
    ...
    mask = cv.bitwise_not(
        cv.circle(np.zeros((np.size(frame, 0), np.size(frame, 1), 1), np.uint8), (iris_circle[0], iris_circle[1]), iris_circle[2], (255, 255, 255), cv.FILLED))
...
def getPolar2CartImg(image, rad):
    ...
    imgRes = cv.warpPolar(image, (rad*3, 360), c,
                          np.size(image, 0)/2, cv.WARP_POLAR_LOG)
...
def generate_data(input_img_path, output_path, last_path):
    ...
    for (path, dir, files) in os.walk(input_img_path):
        ...
        if not(os.path.isdir(output_path + path.split(last_path)[1])):
            os.mkdir(output_path + path.split(last_path)[1])
        for filename in files:
            ...
            if ((ext == '.bmp') or (ext == '.jpg')):
                ...
                cv.imwrite(output_path + path.split(last_path)
                           [1] + "\\" + filename, norm_frame)
...
# For train data
input_img_path = "./dataset/train/"
output_path = "./normalized_dataset_image/"

generate_data(input_img_path, output_path, "train")


#For test data
input_img_path = "./dataset/test/"
output_path = "./predict/"

generate_data(input_img_path, output_path, "test")